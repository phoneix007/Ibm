...
export const teacherSessionSectionReducer = (
	...
	switch (action.type) {
		case TEACHER_TEMP_SET:
			localStorage.setItem(
				"urlParameter",
				JSON.stringify({ ...state.urlParameter, [action.obj]: action.payload })
			);
			return {
				urlParameter: { ...state.urlParameter, [action.obj]: action.payload },
			};
...
export const teacherSessionStatusReducer = (
	state = { SessionStatusInfo: [] },
	action
) => {
	switch (action.type) {
...
export const teacherContentStatusReducer = (
	state = { contentStatusInfo: [] },
	action
) => {
	switch (action.type) {
...
export const teacherAssessmentsReducer = (

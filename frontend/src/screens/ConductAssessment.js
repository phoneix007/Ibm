...
export const ConductAssessmentScreen = ({ history }) => {
	...
	return (
		<>
			<h1 style={{ "text-align": "center" }}>Conduct Assessment</h1>
			{loading ? (
				...
				<div>
					...
					<div
						style={{ margin: "3% 20%", width: "60%", justifyContent: "center" }}
					>
						<label style={{ "margin-right": "5px" }}>
							Select Cohort:
							<select
								onChange={(e) => displayCourses(e.target.value)}
					...
					{courseLoading ? (
						...
						<Message variant="danger">{courseError}</Message>
					) : (
						<div>
							<Table
								striped
